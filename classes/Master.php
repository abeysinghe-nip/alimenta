...
Class Master extends DBConnection {
	...
	function save_product(){
		...
		if($check > 0){
			...
			if(empty($id)){
				$sql = "INSERT INTO `product_list` set {$data}, image_path = 'uploads/products/{$img_path}'";
				$save = $this->conn->query($sql);
				...
				{
					$sql = "UPDATE `product_list` set {$data}, image_path = 'uploads/products/{$img_path}' where id = '{$id}' ";
					$save = $this->conn->query($sql);
	...
	function place_order(){
		...
		$has_failed=false;
		$gtotal = 0;
		$vendors = $this->conn->query("SELECT * FROM `vendor_list` where id in (SELECT vendor_id from product_list where id in (SELECT product_id FROM `cart_list` where client_id ='{$this->settings->userdata('id')}'))");
		$prefix = date('Ym-');
		$code = sprintf("%'.05d",1);
